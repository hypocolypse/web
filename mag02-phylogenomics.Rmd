...
We only got 1 hit for [*Poseidonibacter*](files/mag02-phylogenomics/Poseidonibacter-TaxIDs-for-ngd.txt) but the script returned 48 for [*Arcobacter*](files/mag02-phylogenomics/Arcobacter-TaxIDs-for-ngd.txt).
...
This time we get [198 hits](files/mag02-phylogenomics/Arcobacter_Group-TaxIDs-for-ngd.txt). Since we only need the TaxIDs, we can rerun the command like so:
...
A [metadata file](files/mag02-phylogenomics/Arcobacter-NCBI-METADATA.txt) is also created by `ncbi-genome-download`.
...
This command returns a directory, `01_Arcobacter-NCBI-GENOMES` full of fasta files and the file [`Arcobacter-fasta.txt`](files/mag02-phylogenomics/Arcobacter-fasta.txt), which contains the name and fasta file path for each genome.
...
Next, we need a config file for the **contigs workflow**. This file tells anvi'o what commands to run and how to run them. We run this command to generate a default config file, which can be modified as you see fit.

```bash
anvi-run-workflow -w contigs --get-default-config contigs-default-config.json
```

[Click here](files/mag02-phylogenomics/contigs-default-config.json) to see the file or grab it from the folded code below. Some commands we chose not to run but they remain in the workflow for posterity.
...
scg_tax <- read.table("tables/mag02-phylogenomics/arco-scg-tax.txt", 
                           sep = "\t", header = TRUE)
datatable(
  scg_tax, rownames = FALSE, autoHideNavigation = TRUE, 
  ...
  options = list(
    ...
    dom = "Blfrtip",
    buttons = c("csv", "copy"),
    pageLength = 5, lengthMenu = c(5, 10, 50, 125), 
...
<aside>
You can access our original [external-genomes file here](files/mag02-phylogenomics/external-genomes.txt).
</aside>
...
datatable(
  arc_comp, rownames = FALSE, autoHideNavigation = TRUE, 
...
arc_hmm <- arc_hmm %>% select(genome_name, total_hits, everything()) 

datatable(
  arc_hmm, rownames = FALSE, autoHideNavigation = TRUE, 
...
<aside>
The [external-genomes file](files/mag02-phylogenomics/external-genomes-rrna-screen.txt) to screen for 16S rRNA.
</aside>
...
datatable(
  arc_rrna, rownames = FALSE, autoHideNavigation = TRUE, 
  ...
  options = list(
    ...
    dom = "Blfrtip",
    buttons = c("csv", "copy"),
    pageLength = 5, lengthMenu = c(5, 10, 50, 115), 
    scrollX = TRUE, scrollCollapse = TRUE, scrollY=TRUE, paging=TRUE,
    fixedColumns = list(leftColumns = 1, rightColumns = 0)))
```
...
<aside>
The [external-genomes file](files/mag02-phylogenomics/external-genomes-rrna.txt) to get 16S rRNA genes.
</aside>
...
<aside>
The [external-genomes file](files/mag02-phylogenomics/external-genomes-dereplicate.txt) for dereplication.
</aside>
...
We decided to use [*Sulfurospirillum*](https://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?mode=Info&id=57665), a member of the Campylobacteraceae, as an outgroup for our tree. We basically ran the same pipeline as above, with a few minor changes. The code is included here but the discussion will be kept to a minimum. I am sure by this point you are tired of hearing me drone on and on.

#### 1. Get TaxIDs

```bash
python gimme_taxa.py 57665 -o Sulfurospirillum-TaxIDs-for-ngd.txt

python gimme_taxa.py 57665 -o Sulfurospirillum-TaxIDs-for-ngd-just-IDs.txt \
                           --just-taxids
```
...
```bash
ncbi-genome-download -t Sulfurospirillum-TaxIDs-for-ngd-just-IDs.txt \
                      bacteria \
                      -o 00_Sulfurospirillum_Genbank \
                      --metadata Sulfurospirillum-NCBI-METADATA.txt \
                      -s genbank \
                      -l complete,chromosome
```

We received 10 *Sulfurospirillum* genomes. We used the `-l complete,chromosome` flag so we wouldn't need to sift through a bunch of data. We only need a few genomes for the outgroup.

#### 3. Format genomes

```bash
anvi-script-process-genbank-metadata -m Sulfurospirillum-NCBI-METADATA.txt \
                                     -o 01_Sulfurospirillum-NCBI-GENOMES \
                                     --output-fasta-txt Sulfurospirillum-fasta.txt \
                                     --exclude-gene-calls-from-fasta-txt
```
...
<aside>
Nice genomes. The lowest [completion estimate](files/mag02-phylogenomics/outgroup-completness.txt) is 95.77%. 
</aside>
...
<aside>
And they all have [16S rRNA genes](files/mag02-phylogenomics/Ribosomal_RNAs-HMM_HITS-outgroup.txt). 
</aside>
...
anvi-dereplicate-genomes -e external-genomes-outgroup.txt 
                         ...
                         -o 06_DEREPLICATE/DR_95 -T $NSLOTS
```                         

Good to go. Let's use *Sulfurospirillum halorespirans* (DSM 13726), *Sulfurospirillum deleyianum* (DSM 6946), *Sulfurospirillum multivorans* (DSM 12446), and *Sulfurospirillum barnesii* (SES 3) as the outgroup since these were all 'unique' at 95% similarity. 

# Phylogenomics

Finally, we are ready for phylogenomic analysis. We first need to make a new *external-genomes* file that contains the *Arcobacter* representatives and the *Sulfurospirillum* outgroup. Then we need to setup a phylogenomic Snakemake workflow. Like before, we start by getting a *default config* file for the workflow.
...
Seems like a good idea to select SCG that are in all of the genomes. Of the 71 genes in the HMM profile, [63 were present in all 77 genomes](files/mag02-phylogenomics/arco-phylo-Bacteria_71-HMM_HITS.txt). We used these for our analysis though we could easily select a subset like all Ribosomal genes. 
...
<aside>
Here is the [modified config](files/mag02-phylogenomics/phylogenomics-config.json) file & the [final external genomes](files/mag02-phylogenomics/arco-phylo-external-genomes.txt) file. 
</aside>
...
> Phylogenomic analysis of 73 *Arcobacter* and 4  *Sulfurospirillum* genomes. 

And away she goes. A straight-up phylogenomic approach using SCG (*picked somewhat at random*) as we just did may not be the best approach for scrutinizing closely related genomes. This is because we are not taking into account the degree of [homogeneity](http://merenlab.org/2016/11/08/pangenomics-v2/#concept-of-homogeneity) within gene clusters. A better approach is to calculate homogeneity indices and use the results to guide our choice of genes clusters for the phylogenomic analysis. For this, we will use anvi'o pangenomic analysis. 
...
The pangenome database now has a new collection of SCG and a new phylogenomic tree. The new collection has 17 gene clusters and 1309 genes. 1309 divided by 77 (number of genomes) equals 17. Sweet.
...
anvi-get-sequences-for-gene-clusters \
                        ...
                        --concatenate-gene-clusters \
                        --min-num-genomes-gene-cluster-occurs 77  \
...
INFO
===============================================
Your filters resulted in 17 gene clusters that contain a total of 1309 genes.
...
As expected, 17 gene clusters encompassing a total of 1309 genes. *Note, the order of genes in the concatenate file may differ but the content should be the same.* 
...
*ModelFinder* tested 49 models (it will test up to 546 protein models) and selected LG+F+R4 as the best-fit model based on BIC. We subsequently ran this model with 5000 ultrafast bootstrap approximations[@hoang2017ultrafast].

```bash
iqtree -s ARCOBACTER-proteins-better-core.fa 
       --prefix arcobacterLG+F+R4-5000  
       -m LG+F+R4 -bb 5000 -T $NSLOTS
```

# 16S rRNA gene analysis from MAG02
...
*ModelFinder* tested 30 models (it will test up to 286 DNA models) and selected TVM+F+I+G4 as the best-fit model based on BIC. We subsequently ran this model with 5000 ultrafast bootstrap approximations[@hoang2017ultrafast].
...
Dandy. Now we have a phylogenomic tree of MAG02 with 72 other *Arcobacter* genomes and 4 from *Sulfurospirillum* as the outgroup. We also have a phylogenetic tree of near-full length 16S rRNA genes from the same genomes. 
...
<a href="mag02-phylogenomics.html" rel="prev">N<sup><u>o</u></sup> 7. SOMETHING</a>
</div>
</div>
