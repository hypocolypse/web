...
Metagenomic datasets are often a bit large so you may need to run these workflows on a compute cluster. A personal computer will be insufficiant for many of the steps. We ran our analyses on the Smithsonian cluster (Hydra). For each step in the workflow we include our Hydra job scripts, embeded in a handy dropdown. 
...
Now we are ready to install anvi'o[@eren2015anvi]. If you plan to work with anvio I have a few recommendations.
...
The source code for this page can be accessed on GitHub by [clicking this link](https://github.com/hypocolypse/web/blob/master/build/mg-setup.Rmd).