...
Time to group low abundance taxa into an *Other* catagory for the bar graph. Since we cannot possibly display all `r nrow(means_w)` differnt taxa in one figure, we need to collapse the low abundance groups.  
...
Again, Time to group low abundance taxa into an *Other* catagory for the bar graph. Since we cannot possibly display all `r nrow(means_tm)` differnt taxa in one figure, we need to collapse the low abundance groups.  
...
Time to drill down a little deeper and look at differntially abundant (DA) ASVs between the two oxygen conditions. DETAILS ON METHOD. 
...
Next, we will combine the results of the ISA analysis with the distribution of ASVs across each sample. We are going to do the analysis in anvi'o[@eren2015anvi] using the `anvi-interactive` command. Anvi'o likes database but it also understands that sometimes you don't have a database. So it offers a *manual mode* for all of us slackers out there. If you type this command you can have a look at the relevant pieces we need for the visualiztion, specifically those under the headings `MANUAL INPUTS` and `ADDITIONAL STUFF`. I will go through these out of order so try to keep up ;)
...
Now I want some general data **about the samples** to overlay on the visual. Again, this can be anything. How about a table of alpha diverity metric. We actually have such a table that was generate a ways back up the road. Just need to fix the column names.
...
Turned out this was a little tricky to figure out, but thanks to this [little nifty block of code](https://github.com/joey711/phyloseq/issues/418#issuecomment-262637034) written by [guoyanzhao](https://github.com/guoyanzhao) on the phyloseq Issues forum, it was a piece of cake. The code can be altered to take any rank. See the post for an explaination. 

Anyway, the goal is to sum each taxa at sum rank and present that as a bar chart for each sample in the visualization. Anvi'o has a specific format it needs where each row is a sample and each column is a taxa. Taxa names need the prefix `t_<RANK>!`. For example, `t_class!` should be added for Class rank. 
...
The last piece we need is to generate dedrograms that order the ASVs by their distribution in the samples and the samples by their ASV composition. For this task we will use anvi'o.
...
Time to put all of these pieces together. This get's a little tricky since we do not have a database to start with becasue some of these files can be loaded directly in the interface but some need to be added to a database. When we fire up the interactive in `--manual` mode, we ***must*** give anvi'o the name of a database and it will *create* that database for us. Then we can shut down the interactive, add the necessary data files, and start back up. 
...
With a poupulated database in hand, we can now begin modifying the visual by running the interactive command again. 
